...
async function list(req, res) { res.json({ data: { groups: await service.list(req.user.id, req.query.q) } }); }
async function create(req, res) { res.status(201).json({ data: { id: await service.create(req.user.id, req.body, req.file) } }); }
async function details(req, res) { res.json({ data: { group: await service.details(req.params.id, req.user.id) } }); }
